...
function Display() {
    const [allTodos, setTodos] = useState([]);  //Main Todo
    const [completedTodo, setCompletedTodo] = useState([]);   //Completed Todo
    const [currentEdit, setCurrentEdit] = useState('');     //For title saveing
    const [currentEditedItem, setCurrentEditedItem] = useState('');   // For adding in Todo
    const [isCompleteScreen, setIsCompleteScreen]=useState(false);  //Toggle between todo and Complete
